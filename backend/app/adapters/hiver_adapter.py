...
class HiverAdapter(SiteAdapter):
    site = "hiver"
    ...
    async def search(self, query: str, filters: dict[str, Any], page_cursor: str | None, limit: int) -> dict[str, Any]:
        ...
        q = quote(query)
        url = f"https://capi.hiver.co.kr/v1/web/search/products/{q}?offset={offset}&limit={size}&order=popular&is-first={'true' if offset == 0 else 'false'}&version=2301&total-count=true&service-type=hiver"
        try:
            payload = self._get_json(url, referer=f"https://www.hiver.co.kr/search?q={q}")
        except HTTPError:
        ...
        for row in raw_items:
            ...
            product_url = f"https://www.hiver.co.kr/products/{product_id}" if product_id else "https://www.hiver.co.kr/"
            ...
            items.append(
                ProductItem(
                    ...
                    source_url=f"https://www.hiver.co.kr/search?q={q}",
